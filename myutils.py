...
class Utils():
    ...
    def result_process_new(self, result_show, name, std=False):
        # average performance
        ave_metric = np.mean(result_show, axis=0).values
        std_metric = np.std(result_show, axis=0).values
        ...
        # improvement
        result_show.loc['Perf.'] = ave_metric
        ...
        for _ in result_show.index:
            if _ in ['Ave.rank', 'p-value']:
                result_show.loc[_, :] = [format(round(_, 4), '.4f') for _ in result_show.loc[_, :].values]
